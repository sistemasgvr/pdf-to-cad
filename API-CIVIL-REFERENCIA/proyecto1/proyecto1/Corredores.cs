...
namespace Civil3DBasico
{
    ...
    public class ComandosCorredores
    {
        ...
        public void CrearSuperficieCorredor()
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                try
                {
                    ...
                    // Códigos de enlace REALES del assembly (Top/Datum u otros). Si se hardcodea uno
                    // que no existe, la superficie sale vacía; por eso se listan los que hay.
                    string[] codes = corridor.GetLinkCodes();
                    if (codes == null || codes.Length == 0)
                    {
                        ed.WriteMessage("\nEl assembly no expone códigos de enlace (links). Añade subassembles con códigos (p. ej. Top/Datum).");
                        tr.Abort(); return;
                    }
                    ed.WriteMessage("\nCódigos de enlace disponibles en el corredor:");
                    for (int i = 0; i < codes.Length; i++) ed.WriteMessage($"\n  {i + 1}. {codes[i]}");
                    ed.WriteMessage("\n  0. (TODOS)");
                    PromptIntegerOptions pio = new PromptIntegerOptions("\nNúmero del código (0 = todos):")
                    { LowerLimit = 0, UpperLimit = codes.Length, DefaultValue = 0, UseDefaultValue = true };
                    PromptIntegerResult pir = ed.GetInteger(pio);
                    if (pir.Status != PromptStatus.OK) { tr.Abort(); return; }

                    CivilDB.CorridorSurface cs = corridor.CorridorSurfaces.Add(nombre);
                    if (pir.Value == 0) foreach (string c in codes) cs.AddLinkCode(c, true);
                    else cs.AddLinkCode(codes[pir.Value - 1], true);

                    corridor.Rebuild();

                    int nAgregados = (cs.LinkCodes() != null) ? cs.LinkCodes().Length : 0;
                    tr.Commit();
                    ed.WriteMessage($"\n✓ Superficie de corredor '{nombre}' creada con {nAgregados} código(s) de enlace.");
                    if (nAgregados == 0) ed.WriteMessage("\n(No se agregó ningún código; revisa el assembly.)");
                }
                catch (Exception ex)
                {
                    ed.WriteMessage($"\nError: {ex.Message}");
        ...
        public void ExtraerSolidosCorredor()
        {
            ...
            // Selección MÚLTIPLE de corredores (con filtro a la clase de corredor)
            TypedValue[] filtro = { new TypedValue((int)DxfCode.Start, "AECC_CORRIDOR") };
            PromptSelectionResult sel = ed.GetSelection(
                new PromptSelectionOptions { MessageForAdding = "\nSeleccione uno o varios corredores a convertir en sólidos 3D:" },
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                try
                {
                    ...
                    {
                        CivilDB.Corridor corridor = tr.GetObject(cid, OpenMode.ForRead) as CivilDB.Corridor;
                        ...
                        // false = construye el sólido conectando la sección REAL estación a estación,
                        // respetando huecos (canal hueco). Con true (barrido) se rellenaría el hueco.
                        p.SweepSolidForShape = false;
                        p.ExportLinks = false;

                        try
                        {
                            ObjectIdCollection creados = corridor.ExportSolids(p, db);
                            total += creados.Count; nCor++;
                            ed.WriteMessage($"\n  {corridor.Name}: {creados.Count} sólido(s).");
                        }
                        catch (Exception exc) { ed.WriteMessage($"\n  {corridor.Name}: error ({exc.Message})."); }
                    }

                    tr.Commit();
                    ed.WriteMessage($"\n✓ Total: {total} sólido(s) 3D de {nCor} corredor(es).");
                    if (total == 0)
                        ed.WriteMessage("\n(0 sólidos: revisa que el assembly tenga FORMAS/shapes cerradas, no solo enlaces.)");
                }
                catch (Exception ex)
                {
                    ed.WriteMessage($"\nError: {ex.Message}");
