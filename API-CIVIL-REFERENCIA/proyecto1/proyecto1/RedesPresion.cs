...
namespace Civil3DBasico
{
    ...
    public partial class ComandosPresion
    {
        ...
        public void ListarPiezasPresion()
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                try
                {
                    ...
                    // Recorre TODAS las Parts Lists (así ves en cuál quedó cada material, p. ej. PE/HDPE)
                    ed.WriteMessage($"\n=== Parts Lists de presión en el dibujo: {col.Count} ===");
                    for (int c = 0; c < col.Count; c++)
                    {
                        PresStyles.PressurePartList pl = (PresStyles.PressurePartList)tr.GetObject(col[c], OpenMode.ForRead);
                        ed.WriteMessage($"\n\n>>> Parts List #{c + 1}: '{pl.Name}'");
                        foreach (var dom in dominios)
                        {
                            var partes = pl.GetParts(dom);   // List<PressurePartSize>
                            int cnt = (partes == null) ? 0 : partes.Count;
                            ed.WriteMessage($"\n  --- {dom} ({cnt}) ---");
                            if (cnt == 0) { ed.WriteMessage("\n    (ninguna)"); continue; }
                            foreach (var ps in partes)
                                ed.WriteMessage($"\n    [{ps.PartType}] {InfoPieza(ps)}");
                        }
                    }
                    ed.WriteMessage("\n==========================================");
                    tr.Commit();
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                try
                {
                    ...
                    if (ent is Polyline poly)
                    {
                        ...
                        try
                        {
                            // AVISO: ¿la superficie de referencia cubre TODA la traza?
                            if (net.ReferenceSurfaceId != ObjectId.Null)
                            {
                                ...
                                if (fuera > 0)
                                {
                                    ...
                                    { ed.WriteMessage("\nCancelado. Amplía la superficie, o usa el modo 3D."); tr.Abort(); return; }
                                }
                        ...
                        {
                            ok2D = false;
                            ed.WriteMessage($"\n⚠ El modo 2D (createPipeRun) falló: {exRun.Message}");
                            ed.WriteMessage("\n  Uso el modo TUBO A TUBO como respaldo (sin codos automáticos; añade codos con UNIR/AGREGAR en los quiebres).");
                        }
                        ...
                        // GARANTIZAR el EJE (alignment): si createPipeRun lo generó, se reporta;
                        // si no, se crea uno desde la polilínea y se asocia a los tubos nuevos.
                        string ejeMsg = "";
                        try
                        {
                            ObjectId ejeId = ObjectId.Null;
                            if (net.PipeRuns.Count > 0)
                            {
                                CivilDB.PressurePipeRun runNuevo = net.PipeRuns[net.PipeRuns.Count - 1];
                                if (runNuevo.AlignmentId.IsValid && !runNuevo.AlignmentId.IsNull) ejeId = runNuevo.AlignmentId;
                            }
                            if (ejeId == ObjectId.Null)
                            {
                                var vs = new List<Point3d>();
                                for (int i = 0; i < poly.NumberOfVertices; i++) vs.Add(poly.GetPoint3dAt(i));
                                ejeId = CrearAlineamientoDesdePuntos(ed, db, civilDoc, tr, vs, nombreRun + "-eje");
                                if (ejeId != ObjectId.Null)
                                    foreach (ObjectId id in net.GetPipeIds())
                                        if (!tubosAntes.Contains(id))
                                            try { var pp = tr.GetObject(id, OpenMode.ForWrite) as CivilDB.PressurePipe; if (pp != null) pp.ReferenceAlignmentId = ejeId; } catch { }
                            }
                            if (ejeId != ObjectId.Null)
                            {
                                CivilDB.Alignment al = tr.GetObject(ejeId, OpenMode.ForRead) as CivilDB.Alignment;
                                ejeMsg = al != null ? $" Eje: '{al.Name}'." : " Eje asociado.";
                            }
                        }
                        catch { }
                        ...
                        {
                            // RESPALDO: armar la línea TUBO A TUBO desde los vértices de la polilínea
                            var vs = new List<Point3d>();
                            for (int i = 0; i < poly.NumberOfVertices; i++) vs.Add(poly.GetPoint3dAt(i));
                            if (vs.Count < 2) { ed.WriteMessage("\nLa polilínea no tiene suficientes vértices."); tr.Abort(); return; }
                            int nT = CrearTuberiasPorPuntos(ed, db, civilDoc, tr, net, vs, tubo, capa, nombreRun + "-eje");
                            tr.Commit();
                            ed.WriteMessage($"\n✓ Línea '{nombreRun}' creada tubo a tubo: {nT} tubo(s)" +
                                            (string.IsNullOrWhiteSpace(capa) ? "." : $", capa '{capa}'.") +
                                            "\n  (Sin codos automáticos; usa UNIR_TUBERIAS_PRESION o AGREGAR_ELEMENTO_PRESION en los quiebres.)");
                        }
                    }
        ...
        // =====================================================================
        // EXTRAER_SOLIDO_PIEZA_PRESION — extrae el sólido 3D SOLO de las piezas
        //   SELECCIONADAS (tubos/codos/válvulas), para no duplicar todos los de la red.
        // =====================================================================
        [CommandMethod("EXTRAER_SOLIDO_PIEZA_PRESION")]
        public void ExtraerSolidoPiezaPresion()
        {
            Document doc = Application.DocumentManager.MdiActiveDocument;
            Editor ed = doc.Editor;
            Database db = doc.Database;
            ...
            { MessageForAdding = "\nSeleccione la(s) pieza(s) de la red a presión (tubos/codos/válvulas):" });
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                try
                {
                    ...
                    // Opcional: adjuntar un Property Set a los sólidos generados
                    bool conPS = ElegirPropertySet(ed, tr, db, out ObjectId psdId);

                    int n = 0, nPS = 0, ignoradas = 0;
                    foreach (ObjectId id in ids)
                    {
                        CivilDB.PressurePart parte = tr.GetObject(id, OpenMode.ForRead) as CivilDB.PressurePart;
                        if (parte == null) { ignoradas++; continue; }   // no es una pieza de red a presión
                        Solid3d sol = null;
                        try { sol = parte.Get3dBody(); } catch { }
                        if (sol == null) continue;
                        ...
                        ms.AppendEntity(sol);
                        tr.AddNewlyCreatedDBObject(sol, true);
                        if (conPS) { try { AecPS.PropertyDataServices.AddPropertySet(sol, psdId); nPS++; } catch { } }
                        n++;
                    }

                    tr.Commit();
                    ed.WriteMessage($"\n✓ {n} sólido(s) 3D extraído(s) de las piezas seleccionadas" +
                                    (string.IsNullOrWhiteSpace(capa) ? "." : $" (capa '{capa}').") +
                                    (conPS ? $" Property Set en {nPS}." : ""));
                    if (ignoradas > 0) ed.WriteMessage($"\n({ignoradas} objeto(s) ignorado(s): no eran piezas de red a presión.)");
                }
                catch (Exception ex)
                {
                    ed.WriteMessage($"\nError: {ex.Message}");
        ...
        public void AdjuntarPropertySet()
        {
            ...
            using (Transaction tr = db.TransactionManager.StartTransaction())
            {
                try
                {
                    ...
                    if (fallo > 0)
                    {
                        ed.WriteMessage($"\n⚠ {fallo} objeto(s) NO lo aceptaron. Suele pasar con tubos/estructuras/piezas de red:");
                        ed.WriteMessage("\n  el Property Set solo se adjunta si su 'Applies To' incluye ese tipo de objeto.");
                        ed.WriteMessage("\n  Alternativas: adjúntalo a los SÓLIDOS 3D extraídos, o exporta los datos con EXPORTAR_RED_CSV / EXPORTAR_RED_PRESION_EXCEL.");
                    }
                }
        ...
        // Lee una propiedad de la pieza sin romper si no aplica a ese tipo.
        private static string PropPieza(PresStyles.PressurePartSize ps, CivilDB.PressurePartContextType ctx)
        {
            try { object v = ps.GetProperty(ctx); return v?.ToString(); } catch { return null; }
        }

        // Descripción enriquecida de una pieza: Descripción + Ø nominal + clase de presión (PN) + SDR.
        // Así se ve qué tubo sirve: se elige por DIÁMETRO y por CLASE DE PRESIÓN (PN).
        private static string InfoPieza(PresStyles.PressurePartSize ps)
        {
            string s = ps.Description;
            string dia = PropPieza(ps, CivilDB.PressurePartContextType.DiameterNominal);
            string pn = PropPieza(ps, CivilDB.PressurePartContextType.PressureClass);
            string sdr = PropPieza(ps, CivilDB.PressurePartContextType.Sdr);
            if (!string.IsNullOrWhiteSpace(dia)) s += $"  Ø{dia}";
            if (!string.IsNullOrWhiteSpace(pn)) s += $"  Clase {pn}";
            if (!string.IsNullOrWhiteSpace(sdr)) s += $"  SDR {sdr}";
            return s;
        }
