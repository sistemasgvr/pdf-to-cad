"""Auditoría «no inventar / precisión» del perfil ALCANTARILLADO en los 4 PDFs de prueba.

Uso: python scripts/audit_alcantarillado.py [config] [salida.json]
  config: profile (por defecto: las reglas que tiene recognition hoy) | none | drain |
          water | all | profile_nopj | profile_nocbv (el perfil sin una regla)
Resultado de referencia (2026-09-25, `profile`): 72 hojas con alcantarillado, 859
tramos, 0 sin tinta, 0 «V», cobertura ≥ 99.77 %, 3 «imprecisos» que son el EJE de
tuberías dibujadas con doble línea / contorno (correctos por definición).
Por hoja: tramos, largo, cobertura, sin cubrir, fuera de patrón, segmentos SIN
TINTA (>6 pt fuera de bóveda con <50 % de muestras a ≤2 pt), vértices en «V»
(<73°, no fillet), IMPRECISOS (p90 de la distancia PERPENDICULAR a los guiones
paralelos de su propia capa > 0.75 pt; letras y huecos no cuentan), AB, codos.
"""
...
def _opts(cfg):
    ...
    if cfg == "profile_nopj":
        return dataclasses.replace(rec.UTILITY_GEOM_OPTIONS[U], precise_junctions=False)
    return {
        "none": geom.GeomOptions(),
        "drain": geom.GeomOptions(separate_vaults=True, nearest_vault=True, absorb_inside_runs=True),
        "water": geom.GeomOptions(join_touching_ends=True, gap_turn_blocks=True, markers_on_curves=True),
        "all": geom.GeomOptions(separate_vaults=True, nearest_vault=True, absorb_inside_runs=True,
...
if __name__ == "__main__":
    import fitz
    cfg = sys.argv[1] if len(sys.argv) > 1 else "profile"
    outp = sys.argv[2] if len(sys.argv) > 2 else f"audit_alcantarillado_{cfg}.json"
    jobs = [(cfg, p, i) for p in PDFS for i in range(fitz.open(D + p).page_count)]
    ...
    print(cfg, "hojas con alcantarillado:", len(act), "/", len(rows),
